...
- [Textbooks Are All You Need II: phi-1.5 technical report](https://arxiv.org/abs/2309.05463), 这是出的第二篇工作，相比第一篇工作主要聚焦在coding问题上，这次直接针对reasoning问题。主要有三个模型：phi-1.5-web-only, phi-1.5-web, phi-1.5，web only是通过模型从web类数据中过滤得到textbook like数据，效果已经比同等大小的模型稍好（但只训练了15% token量），phi-1.5-web是在web only基础上再加上合成数据，phi-1.5则不使用web数据只使用合成数据。发现phi-1.5-web与phi-1.5这两个使用合成数据的模型效果大幅提升。
...
- [PanGu-Coder2: Boosting Large Language Models for Code with Ranking Feedback](https://arxiv.org/abs/2307.14936), 提出RRTF(Rank Responses to align Test&Teacher Feedback), 基本思路是分成三个阶段:第一阶段先通过Evol-Instrucct方法获得大量的prompt-response数据，第二阶段将获得代码response实际执行获得编译和执行的反馈结果，以及人类的偏好信号也可以；第三阶段是将prompt-response-score的数据训练student model，本质上其实是一个distill+feedback工作。最后用15B的模型在HumanEval@paas1达到61%，效果还不错。
...
- [Diffusion Policy: Visuomotor Policy Learning via Action Diffusion](https://arxiv.org/abs/2303.04137), 将diffusion的思想应用在offline policy中。
